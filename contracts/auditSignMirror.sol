...
//
// This contract is a mirroring instance, deployed on a different network.
// Every signature that is submitted on the mainnet version will also be
// submitted to this mirroring instance. This way, users who deployed their
// contracts on different networks can reference to a signed audit report.
...
contract auditSignMirror is OwnableUpgradeable {
    ...
    // Each mirroring contract on other networks
    // references the mainnet contract, where users
    // may check it's validity. It's deployed on
    // audits.byterocket.eth
